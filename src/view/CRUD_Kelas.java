...
public class CRUD_Kelas extends javax.swing.JPanel {
    ...
    //menampilkan CRUD_Kelas comboBox pada tambah menu
    ...
    private void updateCombo1() {
        ...
        try {
            PreparedStatement stat = conn.prepareStatement(sql);
            stat.setString(1, "Guru Kelas");
            ResultSet rs = stat.executeQuery();
            while (rs.next()) {
                cbguru.addItem(rs.getString("nama_pegawai"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void updateCombo3() {
        ...
        try {
            ...
            ResultSet rs = stat.executeQuery();
            while (rs.next()) {
                cbguru1.addItem(rs.getString("nama_pegawai"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//    combBox Pilih Kelas
    ...
    private void updateCombo2() {
        ...
        String sql = "SELECT DISTINCT wali_kelas FROM guru WHERE wali_kelas IS NOT NULL";
        try {
            PreparedStatement stat = conn.prepareStatement(sql);
            ResultSet rs = stat.executeQuery();
            while (rs.next()) {
                jkelas.addItem(rs.getString("wali_kelas")); // Tambahkan kembali item dari hasil query
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        // Setel placeholder sebagai item yang dipilih secara default
    ...
    void updateTabelGuru() {
        ...
        // Buat query SQL untuk mengambil data guru berdasarkan CRUD_Kelas yang dipilih
        String sql = "SELECT nama_guru FROM guru WHERE wali_kelas = ?";
        try {
            PreparedStatement stat = conn.prepareStatement(sql);
    ...
    void tampilkanDataSiswa() {
        // Ambil nilai CRUD_Kelas yang dipilih dari combo box jkelas
        String kelas = (String) jkelas.getSelectedItem();

        // Buat query SQL untuk mengambil data siswa berdasarkan CRUD_Kelas yang dipilih
        String sql = "SELECT nipd, nama_siswa, kelamin, tempat_lahir, tanggal_lahir "
                + "FROM siswa "
                + "WHERE kelas = ?";
    ...
    private void simpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_simpanActionPerformed
        ...
        try {
            PreparedStatement stat = conn.prepareStatement(sql);
            stat.setString(1, cbguru.getSelectedItem().toString());
    ...
    private void hapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_hapusActionPerformed
        ...
        if (barisTerpilih == -1) {
            ...
            if (konfirmasi == JOptionPane.YES_OPTION) {
                String sql = "DELETE FROM guru WHERE nama_guru = ?";
                try {
                    // Ambil CRUD_Kelas yang diajar oleh guru yang dihapus
                    String kelasGuru = null;
                    String sqlKelas = "SELECT wali_kelas FROM guru WHERE nama_guru = ?";
                    PreparedStatement statKelas = conn.prepareStatement(sqlKelas);
                    statKelas.setString(1, namaGuru);
                    ResultSet rsKelas = statKelas.executeQuery();
                    if (rsKelas.next()) {
                        kelasGuru = rsKelas.getString("wali_kelas");
                    }

                    PreparedStatement stat = conn.prepareStatement(sql);
                    stat.setString(1, namaGuru);
    ...
    private void simpan2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_simpan2ActionPerformed
        try {
            String sql = "UPDATE guru SET nama_guru=? where wali_kelas=?";
            PreparedStatement stat = conn.prepareStatement(sql);
            stat.setString(1, cbguru1.getSelectedItem().toString());
            stat.setString(2, cbkelas1.getSelectedItem().toString());
