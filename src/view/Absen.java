...
//        // Setel placeholder sebagai item yang dipilih secara default
    ...
    public void tampilkanDataSiswa() {
        ...
        String sql = "SELECT nipd, nama_siswa, kelamin, kelas FROM siswa WHERE kelas = ?";
        ...
        if (kelasSelected.equals("-- Pilih Kelas --")) {
            ...
            if (rowCount == 0) {
                ...
                try {
                    ...
                    Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/simsditp", "root", "");
                    ...
                    if (count == 0) {
                        ...
                        String sql = "INSERT INTO absen (nipd, nama_siswa, kelamin, kelas, presensi, keterangan, tanggal) VALUES (?, ?, ?, ?, ?, ?, ?)";
                        PreparedStatement stat = conn.prepareStatement(sql);
    ...
    private void tambah1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tambah1ActionPerformed
        ...
        if (selectedRow != -1) { // Pastikan ada baris yang dipilih
            try {
                ...
                // Setel nilai parameter untuk query SQL
                stat.setString(1, presensi);
                stat.setString(2, keterangan);
                stat.setString(3, nipd);
